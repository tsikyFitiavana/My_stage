...
Clients.selectionerUnUtilisateur = (pseudo_client,resultat) =>{
    connexion.query('select * from clients where pseudo_client =? ' , pseudo_client, (erreur, response)=>{
        if(erreur) {
            // console.log("erreur de selection : ", erreur);
            resultat(null, erreur);
        }
...
Clients.listerTousLesClient = (resultat) => {
    connexion.query('select * from clients', (err, response)=>{
        if(err){
